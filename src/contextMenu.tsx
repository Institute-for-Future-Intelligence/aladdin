...
const ContextMenu = ({ city, canvas, setPvDialogVisible, ...rest }: ContextMenuProps) => {
  ...
  switch (selectedElement ? selectedElement.type : clickObjectType) {
    ...
    case ObjectType.SolarPanel:
      ...
      const dx =
        solarPanel.orientation === Orientation.portrait
          ? solarPanel.pvModel.width
          : solarPanel.pvModel.length;
      const dy =
        solarPanel.orientation === Orientation.portrait
          ? solarPanel.pvModel.length
          : solarPanel.pvModel.width;
          ...
          <Menu>
                ...
                onChange={(value) => {
                  if (solarPanel) {
                    if (value === Orientation.portrait) {
                      ...
                      setElementSize(
                        solarPanel.id,
                        nx * solarPanel.pvModel.width,
                        ny * solarPanel.pvModel.length,
                      );
                    } else {
                      // calculate the current x-y layout
                      ...
                      setElementSize(
                        solarPanel.id,
                        nx * solarPanel.pvModel.length,
                        ny * solarPanel.pvModel.width,
                      );
                    }
            ...
            <Menu.Item key={'solar-panel-width'} style={{ paddingLeft: '40px' }}>
              <Space style={{ width: '150px' }}>
                {'Width (' + Math.round(solarPanel.lx / dx) + ' panels):'}
              </Space>
              <InputNumber
                min={dx}
                max={100 * dx}
            ...
            <Menu.Item key={'solar-panel-length'} style={{ paddingLeft: '40px' }}>
              <Space style={{ width: '150px' }}>
                {'Length (' + Math.round(solarPanel.ly / dy) + ' panels):'}
              </Space>
              <InputNumber
                min={dy}
                max={100 * dy}
    ...
    case ObjectType.Cuboid:
      const sensorCountCuboid = selectedElement
        ? countAllChildElementsByType(selectedElement.id, ObjectType.Sensor)
        : 0;
      return (
        <StyledMenu>
