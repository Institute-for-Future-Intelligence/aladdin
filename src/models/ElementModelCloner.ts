...
export class ElementModelCloner {
  ...
  private static cloneSolarPanel(parent: ElementModel, solarPanel: SolarPanelModel, x: number, y: number, z?: number) {
    ...
    if (parent.type === ObjectType.Foundation && (parent as FoundationModel).enableSlope) {
      cz = parent.cz + Util.getZOnSlope(parent.lx, (parent as FoundationModel).slope, x);
    }
    return {
  ...
  private static cloneWindTurbine(
    ...
    return {
      ...
      bladeRootRadius: windTurbine.bladeRootRadius,
      bladeTipWidth: windTurbine.bladeTipWidth,
      bladeRadius: windTurbine.bladeRadius,
      hubRadius: windTurbine.hubRadius,
