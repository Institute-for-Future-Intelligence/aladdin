...
const FresnelReflector = React.memo((fresnelReflector: FresnelReflectorModel) => {
  const {
    id,
    cx,
    cy,
  ...
  }, [fresnelReflector?.label, fresnelReflector?.locked, lang, rx, ry, rz]);
    ...
    if (receiver) {
      if (receiver.type === ObjectType.Foundation) {
        ...
        if (foundation.solarStructure === SolarStructure.FocusPipe && foundation.solarAbsorberPipe) {
          ...
          return new Vector3(
            (foundation.cx - rx) * (rot ? Math.cos(rot) : 1),
            (foundation.cy - ry) * (rot ? Math.sin(rot) : 0),
            foundation.cz - rz + foundation.lz / 2 + (foundation.solarAbsorberPipe.absorberHeight ?? 10),
          );
        }
      }
    } else {
  ...
  return (
    <group name={'Fresnel Reflector Group ' + id} rotation={euler} position={[rx, ry, rz + hz]}>
