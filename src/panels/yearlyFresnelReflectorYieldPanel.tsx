...
const YearlyFresnelReflectorYieldPanel = React.memo(({ city }: YearlyFresnelReflectorYieldPanelProps) => {
  ...
  const labelY = t('fresnelReflectorYieldPanel.Yield', lang);
  ...
  return (
    ...
    >
      <Container
        ref={nodeRef}
        style={{ zIndex: selectedFloatingWindow === 'yearlyFresnelReflectorYieldPanel' ? Z_INDEX_FRONT_PANEL : 9 }}
      >
        <ColumnWrapper
          ref={wrapperRef}
          style={{
        ...
        >
          <Header className="handle" style={{ direction: 'ltr' }}>
            <span>
              {t('fresnelReflectorYieldPanel.FresnelReflectorYearlyYield', lang) + ': '}
          ...
          {!simulationInProgress && (
            <Space style={{ alignSelf: 'center', direction: 'ltr' }}>
              {individualOutputs && fresnelReflectorCount > 1 && reflectorSumRef.current.size > 0 ? (
                ...
                <Space>
                  {t('fresnelReflectorYieldPanel.YearlyTotal', lang)}:{(sum * yearScaleFactor).toFixed(2)}{' '}
                  {t('word.kWh', lang)}
                </Space>
              )}
