...
const DailyParabolicDishYieldPanel = ({ city }: DailyParabolicDishYieldPanelProps) => {
  ...
  const panelX = useStore(Selector.viewState.dailyParabolicDishYieldPanelX);
  const panelY = useStore(Selector.viewState.dailyParabolicDishYieldPanelY);
  ...
  const onDragEnd: DraggableEventHandler = (e, ui) => {
    setCommonStore((state) => {
      state.viewState.dailyParabolicDishYieldPanelX = Math.max(ui.x, wOffset - window.innerWidth);
      state.viewState.dailyParabolicDishYieldPanelY = Math.min(ui.y, window.innerHeight - hOffset);
    });
  };
