...
const DailyHeliostatYieldPanel = React.memo(({ city }: DailyHeliostatYieldPanelProps) => {
  ...
  const labelY = t('heliostatYieldPanel.YieldPerHour', lang);
  ...
  return (
    ...
    >
      <Container
        ref={nodeRef}
        style={{ zIndex: selectedFloatingWindow === 'dailyHeliostatYieldPanel' ? Z_INDEX_FRONT_PANEL : 9 }}
      >
        <ColumnWrapper
          ref={wrapperRef}
          style={{
        ...
        >
          <Header className="handle" style={{ direction: 'ltr' }}>
            <span>
              {t('heliostatYieldPanel.HeliostatDailyYield', lang) + ': '}
            ...
            <Space style={{ alignSelf: 'center', direction: 'ltr' }}>
              {individualOutputs && heliostatCount > 1 && heliostatSumRef.current.size > 0 ? (
                ...
                <Space style={{ cursor: 'default' }}>
                  {t('heliostatYieldPanel.DailyTotal', lang)}:{sum.toFixed(2)} {t('word.kWh', lang)}
                </Space>
              )}
              {heliostatCount > 1 && (
                <Switch
...
export default DailyHeliostatYieldPanel;
