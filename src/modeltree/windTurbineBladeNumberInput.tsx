...
const WindTurbineBladeNumberInput = ({ turbine }: { turbine: WindTurbineModel }) => {
  const addUndoable = useStore(Selector.addUndoable);
  const [value, setValue] = useState<number>(turbine.numberOfBlades);

  const lang = useLanguage();
  const { t } = useTranslation();

  useEffect(() => {
    setValue(turbine.numberOfBlades);
  }, [turbine.numberOfBlades]);

  const update = (value: number) => {
    useStore.getState().set((state) => {
      const a = state.elements.find((e) => e.id === turbine.id);
      if (a) {
        (a as WindTurbineModel).numberOfBlades = value;
      }
    });
  };

  const confirm = () => {
    const oldValue = turbine.numberOfBlades;
    if (Math.abs(value - oldValue) < ZERO_TOLERANCE) return;
    const undoableChange = {
      name: 'Set Wind Turbine Blade Number',
      timestamp: Date.now(),
      oldValue,
      newValue: value,
...
export default WindTurbineBladeNumberInput;
