...
const WindTurbinePitchInput = ({ turbine }: { turbine: WindTurbineModel }) => {
  const addUndoable = useStore(Selector.addUndoable);
  const [value, setValue] = useState<number>(turbine.pitchAngle);

  const lang = useLanguage();
  const { t } = useTranslation();

  useEffect(() => {
    setValue(turbine.pitchAngle);
  }, [turbine.pitchAngle]);
  ...
  const update = (value: number) => {
    useStore.getState().set((state) => {
      ...
      if (a) {
        (a as WindTurbineModel).pitchAngle = value;
      }
    });
  };

  const confirm = () => {
    const oldValue = turbine.pitchAngle;
  ...
  return (
    <Space>
      <span>{t('windTurbineMenu.RotorBladePitchAngle', lang)}: </span>
      <InputNumber
...
export default WindTurbinePitchInput;
