...
const WindTurbineBladeRadiusInput = ({ turbine }: { turbine: WindTurbineModel }) => {
  const addUndoable = useStore(Selector.addUndoable);
  const [value, setValue] = useState<number>(turbine.bladeRadius);

  const lang = useLanguage();
  const { t } = useTranslation();

  useEffect(() => {
    setValue(turbine.bladeRadius);
  }, [turbine.bladeRadius]);
  ...
  const update = (value: number) => {
    useStore.getState().set((state) => {
      ...
      if (a) {
        (a as WindTurbineModel).bladeRadius = value;
      }
    });
  };

  const confirm = () => {
    const oldValue = turbine.bladeRadius;
    if (Math.abs(value - oldValue) < ZERO_TOLERANCE) return;
    const undoableChange = {
      name: 'Set Wind Turbine Rotor Blade Radius',
      timestamp: Date.now(),
      oldValue,
      newValue: value,
