...
const RotateHandle = ({ id, position, color, ratio, handleType, hoverHandle, noHoverHandle }: RotateHandleProps) => {
  ...
  return (
    <group position={position} rotation={[Math.PI / 2, 0, 0]} scale={ratio} name={handleType}>
      <group>
        <Torus args={[0.15, 0.05, 6, 8, (3 / 2) * Math.PI]} rotation={[Math.PI / 2, 0, Math.PI / 2]}>
          {rotationHandleLMesh}
        </Torus>
        <Cone args={[0.1, 0.1, 6]} rotation={[Math.PI / 2, 0, 0]} position={[0.15, 0, 0.05]}>
          {rotationHandleLMesh}
        </Cone>
        <Circle args={[0.05, 6]} rotation={[0, Math.PI / 2, 0]} position={[0, 0, 0.15]}>
          {rotationHandleLMesh}
        </Circle>
      </group>
      <Plane
