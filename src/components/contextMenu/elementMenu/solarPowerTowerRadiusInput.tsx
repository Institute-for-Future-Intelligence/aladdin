...
const SolarPowerTowerRadiusInput = ({ setDialogVisible }: { setDialogVisible: (b: boolean) => void }) => {
  ...
  const inputTowerRadiusRef = useRef<number>(powerTower?.towerRadius ?? 1);

  const lang = { lng: language };

  useEffect(() => {
    if (powerTower) {
      inputTowerRadiusRef.current = powerTower.towerRadius ?? 1;
    }
  }, [foundation]);
  ...
  const ok = () => {
    setTowerRadius(inputTowerRadiusRef.current);
    setDialogVisible(false);
    setApplyCount(0);
  };
        ...
        <Row gutter={6}>
          <Col className="gutter-row" span={6}>
            <InputNumber
              ...
              step={0.5}
              precision={1}
              value={inputTowerRadiusRef.current}
              onChange={(value) => {
                inputTowerRadiusRef.current = value;
                setUpdateFlag(!updateFlag);
              }}
              onPressEnter={ok}
            />
