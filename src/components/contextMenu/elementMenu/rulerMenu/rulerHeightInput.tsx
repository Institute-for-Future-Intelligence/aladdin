...
const RulerHeightInput = ({ setDialogVisible }: { setDialogVisible: (b: boolean) => void }) => {
  ...
  const [inputValue, setInputValue] = useState<number>(ruler?.lz ?? 0.1);
  ...
  const needChange = (lz: number) => {
    ...
    switch (actionScope) {
      ...
      default: {
        if (Math.abs(ruler?.lz - lz) > ZERO_TOLERANCE) {
          return true;
        }
        break;
      }
    ...
    switch (actionScope) {
      case Scope.AllSelectedObjectsOfThisType: {
        ...
        const undoableChangeSelected = {
          name: 'Set Lz for Selected Rulers',
          timestamp: Date.now(),
