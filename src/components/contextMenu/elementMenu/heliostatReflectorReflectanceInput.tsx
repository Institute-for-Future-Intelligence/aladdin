...
const HeliostatReflectanceInput = ({
  dialogVisible,
  setDialogVisible,
}: {
  ...
  const [inputReflectance, setInputReflectance] = useState<number>(heliostat?.reflectance ?? 0.9);
  ...
  useEffect(() => {
    if (heliostat) {
      setInputReflectance(heliostat.reflectance);
    }
  }, [heliostat]);
