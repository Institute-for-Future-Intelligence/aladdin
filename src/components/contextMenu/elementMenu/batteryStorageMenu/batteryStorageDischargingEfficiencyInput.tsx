...
const BatteryStorageDischargingEfficiencyInput = ({ setDialogVisible }: { setDialogVisible: (b: boolean) => void }) => {
  ...
  const [inputValue, setInputValue] = useState<number>(100 * (batteryStorage?.dischargingEfficiency ?? 0.95));
  ...
  const needChange = (dischargingEfficiencyPercent: number) => {
    ...
    switch (actionScope) {
      case Scope.AllSelectedObjectsOfThisType: {
        for (const e of elements) {
          if (e.type === ObjectType.BatteryStorage && !e.locked && selectedElementIdSet.has(e.id)) {
            ...
            if (
              Math.abs((battery.dischargingEfficiency ?? 0.95) * 100 - dischargingEfficiencyPercent) > ZERO_TOLERANCE
            ) {
              return true;
            }
          }
      ...
      default: {
        if (
          Math.abs((batteryStorage?.dischargingEfficiency ?? 0.95) * 100 - dischargingEfficiencyPercent) >
          ZERO_TOLERANCE
        ) {
          return true;
        }
      ...
      case Scope.AllSelectedObjectsOfThisType: {
        ...
        for (const elem of elements) {
          if (elem.type === ObjectType.BatteryStorage && !elem.locked && selectedElementIdSet.has(elem.id)) {
            ...
            oldSelected.set(elem.id, battery.dischargingEfficiency);
          }
        }
