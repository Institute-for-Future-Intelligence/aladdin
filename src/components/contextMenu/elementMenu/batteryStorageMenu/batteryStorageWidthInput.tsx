...
const BatteryStorageWidthInput = ({ setDialogVisible }: { setDialogVisible: (b: boolean) => void }) => {
  ...
  const [inputValue, setInputValue] = useState<number>(batteryStorage?.ly ?? 0);
  ...
  const needChange = (ly: number) => {
    ...
    switch (actionScope) {
      ...
      default: {
        if (Math.abs(batteryStorage?.ly - ly) > ZERO_TOLERANCE) {
          return true;
        }
        break;
      }
  ...
  const updateLyById = (id: string, val: number) => {
    useStore.getState().set((state) => {
  ...
  const setLy = (value: number) => {
    ...
    switch (actionScope) {
      ...
      case Scope.AllObjectsOfThisType: {
        ...
        for (const elem of useStore.getState().elements) {
          if (elem.type === ObjectType.BatteryStorage && !elem.locked) {
            oldValAll.set(elem.id, elem.ly);
          }
        }
        const undoableChangeAll = {
          name: 'Set Ly for All Battery Storages',
          timestamp: Date.now(),
          oldValues: oldValAll,
          newValue: value,
          undo: () => {
      ...
      case Scope.AllObjectsOfThisTypeAboveFoundation: {
        ...
        const undoableChangeAll = {
          name: 'Set Ly for All Battery Storage on Same Foundation',
          timestamp: Date.now(),
          oldValues: oldValAll,
          newValue: value,
      ...
      default: {
        const undoableChange = {
          name: 'Set Battery Storage Ly',
          timestamp: Date.now(),
          oldValue: batteryStorage.ly,
          newValue: value,
        ...
        updateLyById(batteryStorage.id, value);
        setApplyCount(applyCount + 1);
        break;
      }
    }
...
export default BatteryStorageWidthInput;
