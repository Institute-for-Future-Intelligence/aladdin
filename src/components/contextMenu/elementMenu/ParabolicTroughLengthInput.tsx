...
const ParabolicTroughLengthInput = ({
  ...
  const [moduleLength, setModuleLength] = useState<number>(3);
  ...
  const needChange = (lx: number) => {
    switch (solarPanelActionScope) {
      ...
      case Scope.AllObjectsOfThisTypeAboveFoundation:
        for (const e of elements) {
          if (e.type === ObjectType.SolarPanel && e.foundationId === parabolicTrough?.foundationId && !e.locked) {
            const trough = e as ParabolicTroughModel;
            if (Math.abs(trough.lx - lx) > ZERO_TOLERANCE) {
              return true;
            }
      ...
      case Scope.AllObjectsOfThisTypeAboveFoundation:
        if (parabolicTrough.foundationId) {
          ...
          if (rejectRef.current) {
            ...
            for (const elem of elements) {
              if (elem.type === ObjectType.SolarPanel && elem.foundationId === parabolicTrough.foundationId) {
            ...
            updateSolarPanelLxAboveFoundation(parabolicTrough.foundationId, value);
            setApplyCount(applyCount + 1);
          }
        }
        break;
      default:
        if (parabolicTrough) {
          ...
          if (rejectRef.current) {
            ...
            updateSolarPanelLxById(parabolicTrough.id, value);
            setApplyCount(applyCount + 1);
          }
        }
    }
  ...
  const panelize = (value: number) => {
    let w = value ?? 1;
    const n = Math.max(1, Math.ceil((w - moduleLength / 2) / moduleLength));
    w = n * moduleLength;
    return w;
  };
        ...
        <Row gutter={6}>
          <Col className="gutter-row" span={6}>
            ...
            <div style={{ paddingTop: '20px', textAlign: 'left', fontSize: '11px' }}>
              {Math.round(inputLength / moduleLength) + ' ' + i18n.t('solarPanelMenu.PanelsWide', lang)}
              <br />
