...
const HeliostatTowerSelection = ({
  ...
  const updateById = useStore(Selector.updateHeliostatTowerById);
  const updateAboveFoundation = useStore(Selector.updateHeliostatTowerAboveFoundation);
  const updateForAll = useStore(Selector.updateHeliostatTowerForAll);
  ...
  const [inputTowerId, setInputTowerId] = useState<string>(heliostat?.towerId ?? 'None');
  ...
  useEffect(() => {
    setInputTowerId('None');
    if (heliostat) {
      if (heliostat.towerId) {
        setInputTowerId(heliostat.towerId);
      } else {
    ...
    switch (actionScope) {
      ...
      case Scope.AllObjectsOfThisTypeAboveFoundation:
        if (heliostat.foundationId) {
          ...
          updateAboveFoundation(heliostat.foundationId, value);
          setApplyCount(applyCount + 1);
        }
        break;
      default:
