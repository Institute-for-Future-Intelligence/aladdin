...
const WindowOpacityInput = ({ setDialogVisible }: { setDialogVisible: (b: boolean) => void }) => {
  ...
  const [input, setInput] = useState<number>(windowModel?.opacity ?? 0.5);
  ...
  const updateById = (id: string, input: number) => {
    setCommonStore((state) => {
      for (const e of state.elements) {
        if (e.id === id) {
          (e as WindowModel).opacity = input;
          break;
        }
      }
    ...
    switch (windowActionScope) {
        ...
        setCommonStore((state) => {
          for (const e of state.elements) {
            if (e.type === ObjectType.Window && !e.locked) {
              oldValuesAll.set(e.id, (e as WindowModel).opacity);
              (e as WindowModel).opacity = value;
            }
          }
        });
        const undoableChangeAll = {
          name: 'Set Window Opacity for All Windows',
          timestamp: Date.now(),
          ...
          setCommonStore((state) => {
            for (const elem of state.elements) {
              if (elem.type === ObjectType.Window && elem.parentId === windowModel.parentId && !elem.locked) {
                oldValues.set(elem.id, (elem as WindowModel).opacity);
                (elem as WindowModel).opacity = value;
              }
            }
          });
          ...
          setCommonStore((state) => {
            for (const elem of state.elements) {
              if (elem.type === ObjectType.Window && elem.foundationId === windowModel.foundationId && !elem.locked) {
                oldValuesAboveFoundation.set(elem.id, (elem as WindowModel).opacity);
                (elem as WindowModel).opacity = value;
              }
            }
          });
          const undoableChangeAboveFoundation = {
            name: 'Set Window Opacity for All Windows Above Foundation',
            timestamp: Date.now(),
      ...
      default:
        if (windowModel) {
          ...
          const oldOverhangLength = updatedWindow.opacity ?? windowModel.opacity ?? 0.5;
...
export default WindowOpacityInput;
